...
Recorded 2026-08-04 against ICE Detention Pathway 3.0.0.
Items marked FIXED were resolved in 3.1.0; see README-v2.md. Unmarked items
remain open.

Counts come from the **locally filtered** copy of the data (arrests reduced to
23,909 NYC-AOR ICE rows). National files will produce larger numbers and may
expose issues this copy cannot show. Every count is reproducible with `q.py`.
...
### A4 — One-arrest/one-stay shortcut skips the time check · P3

`ice_detention_pathway.py:345` pairs a lone arrest with a lone stay
unconditionally. Every other pairing requires the arrest to precede the stay's
book-in within `ARREST_TO_BOOK_IN_TOLERANCE`.

So a single arrest years after a single stay still pairs, and reports a
`DISCREPANCY` rather than the more accurate `NO ARREST RECORD IN THIS DATASET`
plus `[ARREST WITH NO RECORDED DETENTION]`.

Deliberate — it avoids splitting the common case into two confusing blocks —
but it is an inconsistency, and it is one source of the A-grade noise in C1.
...
### A6 — Stay metadata is taken from mismatched ends · P3

`group_stays` reads `entry_program`/`entry_aor` from the **first** stint
(`:324-325`) and `release_reason` from the **last** (`:318`). Defensible, but
undocumented in the output. A stay transferred between AORs shows only its
opening AOR, and a reader cannot tell that later stints disagreed.
...
**Fix:** name the fields instead of narrating them, e.g.
`(stint fields — program: Border Patrol; book-in AOR: Houston Area of Responsibility)`.
...
### B3 — A `stay_ID` or `stint_ID` input is silently widened · P2

`normalize_identifier` truncates at the first underscore, so pasting a specific
`stay_ID` returns **every** stay for that person. Documented, but it means a
user asking about one stay is answered about all of them — the opposite of the
scoping the 3.0.0 fix was about.

**Fix:** when the input carries a stay or stint suffix, render that stay and
mark the others as context.
...
### C4 — 835 rows flagged `duplicate_drop_row`, none excluded · P2 · FIXED in 3.1.0

DDP flags 835 detention rows as duplicates and excludes them from its own
`n_stints`. This project reads every row, so stint counts here can exceed DDP's
for the same stay.

Left deliberately: silently dropping rows contradicts the project's
retain-and-label rule. But the divergence is undocumented, and a user comparing
against DDP's published figures will see a mismatch with no explanation.

**Fix:** keep the rows, label them.

### C5 — 708 stints have no `final_program` · P2

The note collapses to a bare `NO ARREST RECORD IN THIS DATASET` with no context.
Correct behaviour; recorded because it is the silent path through B1.

### C6 — 3 stays have an open stint that is not the last one · P3

1,695 stints have no book-out. In 3 stays the open stint is followed by a later
one, so `Stay.end` returns `None` and the stay reads as ongoing even though a
later stint closed. Rare, and arguably the honest reading, but it means
"currently held" can appear on a stay with a subsequent completed placement.

### C7 — Local data is filtered and cannot say so · P1

`arrests-latest.parquet` is 23,909 rows, 100% `New York City Area of
Responsibility`, 100% `arresting_agency = ICE`. The NYC filter overwrote the
originals.

Consequences that no search result can reveal:

- Anyone arrested outside NYC AOR is unfindable.
- `people_with_detention_but_no_arrest` reads 0 here purely because arrests and
  stints were filtered together. On national data it is a large population, and
  it is the group the 3.0.0 detention-first lookup exists to serve.
- Whether a missing arrest row means "CBP, so ICE never recorded one" or
  "filtered out" is **undecidable from this copy**. Both produce identical
  output.

**Fix:** re-download all files from the Deportation Data Project. Keep backups.
Filtering cannot be reversed.
